...
func GetResultsFromDistrict(db *pgxpool.Pool, district int) ([]ResPresidencial, error) {
	...
	rowsFirstQuery, err := db.Query(ctx, "SELECT votos_validos, votos_blancos, votos_nulos, votos_total FROM recinto t1, mesa t2, candidatura t3, votos_por_candidatura t4 WHERE t1.id = t2.id_recinto AND t2.id = t4.id_mesa AND t3.id = t4.id_candidatura AND t3.id = 1 AND t4.id_candidatura = 1 AND t2.circunscripcion = $1", district)
	if err != nil {
		fmt.Println(err)
	}
	...
	for _, partido := range partidos {
		...
		rows, err := db.Query(ctx, "SELECT cantidad FROM recinto t1, mesa t2, candidatura t3, votos_por_candidatura t4, votos_por_partido t5 WHERE t1.id = t2.id_recinto AND t2.id = t4.id_mesa AND t3.id = t4.id_candidatura AND t4.id_mesa = t5.id_mesa AND t4.id_candidatura = t5.id_candidatura AND t3.id = 1 AND t4.id_candidatura = 1 AND t5.id_candidatura = 1 AND t2.circunscripcion = $1 AND t5.id_partido = (SELECT id FROM partido WHERE sigla = $2)", district, partido)
		if err != nil {
			fmt.Println(err)
		}
