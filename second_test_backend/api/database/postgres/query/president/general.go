...
func GetResults(db *pgxpool.Pool) ([]ResPresidencial, error) {
	ctx := context.Background()

	rowsFirstQuery, err := db.Query(ctx, "SELECT votos_validos, votos_blancos, votos_nulos, votos_total FROM votos_por_candidatura WHERE id_candidatura = 1")
	if err != nil {
		fmt.Println(err)
	}
	...
	for _, partido := range partidos {
		...
		rows, err := db.Query(ctx, "SELECT cantidad FROM votos_por_partido WHERE id_candidatura = 1 AND id_partido = (SELECT id FROM partido WHERE sigla = $1)", partido)
		if err != nil {
			fmt.Println(err)
		}
